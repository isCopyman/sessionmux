...
信已持久化
    │
    ├─ 写给 human/user ─────────────── 悬浮窗 / 通知（不启动任何 Session）
    │
    └─ 写给某个 Session
           │
           ├─ 勿扰中 ──────────────── 不推送；Agent 仍可自己读 inbox
           ├─ 正在跑一轮 ──────────── Hook / checkpoint 注入短提醒
           │                           （有未读 / 已读未回；不打断、不重发正文）
           ├─ 已连接且空闲 / 休息 ─── Codeg 直接开一轮，把信交进去
           │                           （强制启动这一轮处理，不是取消当前任务）
           └─ 关掉了 ──────────────── 不冷启动；信留着；提醒人去打开
```
...
- **跑着的时候**不能硬打断（训练、长工具），所以用 Hook 让它知道有信。
- **空闲或休息时**再不启动，信就会一直躺着，所以 Codeg 必须能自己 `session/prompt`。
- 关掉的 Session 默认不偷偷 Resume；那是冷启动，要人点头。
