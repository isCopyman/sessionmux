...
| O13 | human @ 房主被吞 | **已修**（待重启激活） |
| O14 | dev 重启记录 | 记录 |
| O15 | agent 建群侧边栏隐身 | **已修**（前端已活；打戳/回填待重启） |
...
| O23 | 路径绑定全程隐形 | 在飞（w-roomux 任务2/3/4） |
| O24 | Room 工具调用渲染之争 | 裁决已定（不内联；活动桥排队） |
| O25 | grok 重启后恢复失忆 | **已修**（待重启激活） |
| O26 | Room 面板无 Ctrl+F | 在飞（w-roomux 任务1） |
| O27 | Ctrl+K 文件搜索不自明 | 排队（交互清扫） |
