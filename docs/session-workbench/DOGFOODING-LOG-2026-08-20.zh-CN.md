...
| O21 | Room 时间线全量渲染卡顿 | **已修**（VirtualizedTranscript 接管，event.id 稳定 key；待重启激活）；@ 弹层渐进渲染另一半见 O38 |
...
| O34 | 会话跨台不唯一、无聚焦跳转、工作状态不可见 | **已修·三层全**（跨台唯一+聚焦+忙碌点/"N 个在忙"徽标；待重启激活；Room 成员活动聚合另记） |
...
| O28 | 群聊打开滚动条在顶部 | **已修**（打开落底+跟随最新+向上翻页保视口；待重启激活） |
| O29 | 群聊缺时间线/日期分隔 | **已修**（withDateSeparators 日期栏；Today/Yesterday 十语键记小尾巴；待重启激活） |
