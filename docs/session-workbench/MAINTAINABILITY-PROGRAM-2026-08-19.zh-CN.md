...
> **历史执行日志，已经冻结。** 本文保留 2026-08-19 起的决策与事故证据，但其中的
> Active/待办/派工状态不再代表当前工作；当前状态只看
> [`NOW-BOARD.zh-CN.md`](NOW-BOARD.zh-CN.md) 和实际代码。尤其不要照本文旧的并行派工安排
> 新任务。
>
...
- 2026-08-20 凌晨 **体验批 CDP 真机验收全过 + release #5**：① 成员数按钮
  （"2 位成员"）点击开成员面板 ✅；② 移除成员出 AlertDialog（文案实测："把
  Session C 移出这个群聊？…时间线上的消息仍会保留…"），取消有效弹层消失 ✅；
  ③ 附加路径端到端：房间"…"菜单（真机菜单项=重命名群聊/加入 Session/管理路径/
  删除群聊）→ 管理附加路径对话框加 docs 目录 → `@MAINTAINABILITY` 文件组命中
  附加根下的台账文档（绑定 folder 是论文目录，命中文件来自新加 root——多 root
  合并真机生效）→ 删路径回"暂无附加路径" ✅；④ 全程零控制台错误。窗格放大真机
  只做冒烟（分屏深交互留用户日用+重命名弹窗双击边角）。探针教训三则：Radix 菜单
  必须 CDP 真实鼠标事件（合成 click 打不开）；页面有 12 个 haspopup 按钮，"取
...
- 2026-08-20 早 **搜索体系四问入交接单（P2）**：用户连珠提问——① 全局搜索覆盖会话+
  文件很奇怪（会话已挪会话中心，是否该撤出）；② 会话中心能否搜 Room（用户倾向 Room
...
- 2026-08-20 夜 **第二轮验收合并 + 全绿**：316 实际早已完工（5a44a8e8），
  死锁在"Room MCP 未起→交付帖发不出→宿主无限重投"（摩擦 12 故障模式的
  完整活体样本）；领导以人类操作员身份直发消息代传交付信息给协调者
  （叮嘱：收口帖不 @ 人防幽灵、验收走主线直做），协调者 3 分钟内复核
  合并包 D（b5fab6d3）。领导验收合并主线（9250ed6b）：provider_anchor
  纯增量字段（models/message.rs）+ 14 家 parser 链尾锚点 + E/F 文档，
  切片 2（forkAtMessage）硬前置解锁。门禁：fmt/三路 clippy 0、server
...
- 2026-08-20 深夜 **w-contextmenu 落地 + 用户连报四摩擦 + UI 交互清扫立项**：
  ① w-contextmenu 合并（48fc210b）：document 冒泡相全局拦截原生右键菜单，
  三例外（可编辑元素/data-native-context-menu/dev Shift）。工人的承重发现：
  Radix 触发器 checkForDefaultPrevented=true，**捕获相拦截会杀掉全站自定义
  右键菜单**——冒泡相+只取消默认动作是唯一正确挂法，测试锁定；既有 6 处
  自定义菜单逐一核对不破。前端门禁绿：vitest **4725**（+6）、eslint 0 错、
  build 过（Rust 零改动沿用上轮）。HMR 即时生效。② 用户修正 O20 复现条件
  （只在文件面板参与时出现——拓扑问题不是全把手问题）；O21 @ 卡"搜索中"
  未能现场复现（探测时已消失），静态定性+两假说记档；O22 启动器加建群
  入口采纳；O23 路径绑定链路 DB 实测全通（assign_to_collection 联动
  root_folder_id 属实）但界面全程隐形——修可见性不修机制。③ 立项
...
- 2026-08-21 凌晨 **重启 #2 + 编队复活 + w-roomux 落地**：① 温重启程序首次
  执行：静默哨兵等 Session A-v2 回合收口（1 分钟）才拔电源，90 秒起新实例。
  回填迁移已跑（三房归属齐全，用户手选值未被碰）。② O13 端到端实测：人类
  Room @ 房主 → delivery 生成、义务挂账、消息 embedded 进协调者回合。
  ③ O25 端到端实测：00:42 催办扫描给三 grok 工人排队唤醒，三工人全部
  in_progress 带忆复活——O30 定性"催办静音链"（催办→唤醒失败→队列卡住→
  短路静音）随 grok 修复自解，短路加固列小任务。④ w-roomux 合并
  （66ae1d6b）：Room Ctrl+F（CSS Custom Highlight API 真高亮，免降级）、
  绑定路径三态显示、附加路径复用 DirectoryBrowser、挪动刷新实证本就通；
  门禁全绿一次过（vitest 369 文件、eslint 0 错、build 过）。工人顺手上报
  reload↔publish_room 自馈环嫌疑（未动，待查）。⑤ O28-O31 立项与裁决：
  滚动锚定、时间线并入骨架、催办裁决（自动催=该有，自动替回=不做）、
  待回复语义（带 @ 才挂账、expects_reply 显式声明默认 false、无结构性
...
- 2026-08-21 凌晨 **prompt 全量审计（零遗漏）+ 三项新裁决**：① 用户要求核对
  ...
  （O31 前半），10 语言。
- 2026-08-21 凌晨 **用户三项拍板落地 + 30 条 prompt 二次审计**：① 审计扩窗到
  最近 30 条：29 条真实 prompt（1 条系 agent 报告剔除）全部在档，与上轮 22 条
  交叉印证零遗漏；前段 5 条对应 O11/O12（已修）、产品目标授权（记忆）、
  provider 双轨（已落地+会话级切换排队）。② **重构提前**（用户原话"是否先
  进行重构和后端模型以及抽象构建，防止各种小bug"→拍板执行）：批次顺序改为
  w-replycopy 收口 → **转录面骨架**（旗舰，Room 迁移，一并吃掉 O21/O28/O29）
  → 交互原语（O17余量/O19/O33）→ 布局模型（O20）。骨架前置调研已派
  r-scaffold（k3 只读：能力清单/耦合点/最小骨架边界/风险）。③ hooks 复议：
  维持 2026-08-19 "不引入"定案——跨家中途必达在协议层无解（claude/codex
  私约可用；不支持的家硬塞产无主回合 #934），现有阶梯（排队保底+high 自动
  steer+显式 send_interrupt+mailbox embedded）已覆盖 claude 系的"及时传入"
  （用户今日体验到的正是这条链）；**重开触发条件**：某家上游正式支持 hook
  （如 kimi #2370）后按家单独评估。④ GitHub：用户倾向接入；领导确认就绪但
  推送需用户明示三件（远端地址/私有与否/解除永不 push 军规），明示前本地
  状态板为准，commit 引用 O# 惯例即日生效。⑤ O33 记档：工作台内 Room 行右键
  仅"关闭"与会话行不对等（部分因后端无 Room 换工作台 API），归交互原语批。

### 5.x 断点快照（2026-08-21 01:4x，用户压缩上下文前落盘）

**在飞 agent（压缩后凭此接续，勿重复派）**：
- **w-replycopy**（opus，worktree）：需要回复/等待回复改方向自明措辞+tooltip，
  10 语言（O31 前半）。已通过 SendMessage 捎入用户措辞偏好：口语化，
  "已读未回"方向；用户两句长候选（"有已读未回消息？"/"发出去的消息还没
  被回复？"）可作 tooltip 解释文案。交付后照旧：验基底→合并→前端门禁。
- **r-scaffold2**（haiku 档，只读侦察）：message-list-view.tsx 能力清单/
  rooms-page 时间线差距/最小骨架边界/迁移风险。**k3[1m] 当晚故障**
  （"model may not exist or no access"），首派 r-scaffold（explore-k3）
  失败后换 haiku 档重派。交付物供领导做骨架设计，不直接施工。

**压缩后行动队列（用户已拍板重构优先）**：
1. 收两个在飞 agent → w-replycopy 验收合并门禁。
2. 依 r-scaffold2 地图，领导设计**转录面骨架**并派工（旗舰；Room 迁上骨架，
   连带解决 O21 渐进渲染/O28 滚动锚定/O29 日期分隔）。
3. 交互原语批（O17 拖拽余量+shift 复制语义/O19/O33，含后端 Room 换台 API）。
4. 布局模型（O20）。
5. 下个重启窗口只欠 **O32 worktree 挂靠**（7670aa28 之后无其他未激活 Rust）。
6. 挂起待用户：GitHub 三件套明示（远端/私有/解除永不 push）→ 届时开放 O 条目
   迁 issues+工人走 PR；产品改名拍板。

**改名候选表（领导 shortlist，供用户拍板，定名前须查商标/域名）**：
| 名字 | 意象 | 契合点 |
|---|---|---|
| Skein | 一束纱线+雁阵 | 多会话线程拧成一束+编队飞行，双关 |
| Baton | 指挥棒+接力棒 | 编排+任务交接一词双义 |
| Camerata | 佛罗伦萨音乐家同好会 | 大师们聚在一个 Room 共创，有典故 |
| Troupe | 剧团 | 一班角色共演一出戏 |
| Fugue | 赋格 | 多声部围绕同一主题交织（瑕疵：另义"神游"） |
| Atelier | 大师工坊 | 协调者带工人出活 |
| Ensemble | 合奏 | 意最正但词太常见 |
筛选原则：避开 Crew/Swarm/Maestro/Loom 等被占心智词；短、好念、中文不别扭。
用户自提 codeteams/codeT 偏功能描述缺记忆钩子；sessionmux 工程味重（用户自评）。
改名执行=机械清扫（binary/bundle id/全文档）单独立项，拍板前不动。
...
### 5.18 2026-08-21 夜：交互行为契约 v1 落库 + 今晚目标（用户指令）

- **INTERACTION-CONTRACT.zh-CN.md v1**：五条铁律（同类同能力/一个家/点击
  去现场/右键灰显不隐藏/自明性）+ 实体×宿主行为矩阵（每格标 ✅/O#/❓）+
  四个待拍板空位。此后 UI 修复一律以"契约偏差"立项、对契约验收；这补上
  了用户多次追问"到底应该是什么行为"而一直缺失的规范层。
- **今晚目标（用户指令"把影响体验的都修了"，领导设定 DoD）**：
  必达 = O33 Room 换台+右键对齐、O34 跨台唯一+聚焦+状态可见、骨架阶段一
  +阶段二（O21/O28/O29）、O19 会话中心同构、O31 后半 N/M 进度、重启激活
...
### 5.21 2026-08-21 清晨：七号终批全绿 + 重启 #4 + 领导 CDP 走查收官

- **七号终批 10/10 全绿**（fmt/clippy×3/test×2/eslint/tsc/vitest/build），
  其中 TEST-DESKTOP 是这台机器上桌面测试面**有史以来第一次真实跑通**
  （O50 修复 + 快照接受后 9/9）。主干 b1b928c4。
- **快照接受**：claude_detail 快照补上 fork 第三轮的 provider_anchor 字段
  ——桌面快照测试此前从未在本机执行过，漂移一直未被评审。
- **重启 #4**：杀旧 next dev（15804）与 codeg.exe（93728），
  `pnpm tauri dev` 全新拉起（新后端含 O4/O33 API），CDP 9222 就绪。
- **走查实测**（契约验收面）：O21 ✅54 行虚拟化；O28 ✅打开落底
  distFromBottom=0；O29 ✅本地化日期栏"2026年8月18日"；O33 ✅右键三项
  齐全+灰显不隐藏+**换台实弹往返两遍**（→Main→复原，toast/分组/目录刷新
  全对）；O19 ✅Room 行全列（成员数/Collection/所在工作台/未读 191/folder/
  分支占位/时间）。O34 忙态点已合、像素级验证待有真实在跑回合。O47 防复燃
  横幅（队列已暂停 + 连接失效提示）在位。
- 今晚 DoD 必达清点：O33 ✅、O34 ✅（三层）、骨架两阶段 ✅（O21/O28/O29）、
  O19 ✅、O31 后半 ✅（昨夜）、重启激活+走查 ✅。争取项：O20 ✅、O2 ✅；
  O17 拖拽与 O8 未动（如实记）。

### 5.22 2026-08-21 晨：扫尾三单（O29 十语/O8 审计/契约升级）后收工

- O29 小尾巴：Today/Yesterday 走 i18n 十语（注入式可选参，纯函数不引
  next-intl），八号批前端四条全绿后合并。
- O8 只读审计交付并落库（O8-TRANSIENT-RETRY-AUDIT.zh-CN.md）：根因=
  `session/prompt` RPC 失败被 `?` 升级成连接死亡；A0+B-lite 一刀方案就绪，
  A1/B-full 默认不做；动回合状态机，凌晨不施工，待用户拍板。
- 契约新增待拍板 5/6：会话中心"最近活动排序"与"一条递减的钟"设计相撞
  （升级为排序+时间列一起定）；工作台 facet 对 Room 的回答前提已被 O33
  推翻。
