...
## ⚠️ 2026-08-21 夜核查：下面的 P0/P1/P5/P6 有一半已经做完了

对着代码逐条核过（不是看文档）。**别照着下面的清单派工**，先读
`NOW-BOARD.zh-CN.md` 的「已核实为过期」。已完成的：P0（IME #518，commit `0c85ff8f`
+ 专门测试）、P1（@ 搜索缓存已在 `use-reference-search.ts`）、
P5a（Room 欠回复已聚合，但用的是独立字段 + 侧栏相加，**照本文描述去做会双重计数**）、
P5b（死代码已删）、P6a（本批完成）、P6b（`lib.rs:905` 已有）。

本文保留原文作为当时的判断记录，不改正文。
