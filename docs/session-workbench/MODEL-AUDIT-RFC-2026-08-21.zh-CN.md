...
- **前置盘点完成，裁决修订（2026-08-21，报告
  `SERDE-PERSISTENCE-INVENTORY.zh-CN.md`）**：盘出 16 类红项（已落盘、
  改名=迁移）——含任务/自动化 config（`Value` 原样入库：**改线=改盘**）、
  队列草稿（外 camel 内 snake 冻结混用）、协作信封键、ACP 转录 JSONL、
  备份 manifest、pet.json 等第三方冻结形状。**"全局统一改名"取消**，
  切面 ⑤ 改为三条硬规则：① 红项冻结当前 casing，新字段只加
  `#[serde(default)]`；② 新的纯线 DTO 一律 camelCase（对齐 handlers/
  协作族）；③ 第三方形状划出名单。真正的收尾动作 = 给红项结构体补
  round-trip 样本测试（CI 钉死 casing），不写迁移脚本。
