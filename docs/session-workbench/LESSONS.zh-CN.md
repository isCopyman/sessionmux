...
## L15 —— 「只生效一次」几乎总是索引/绑定丢了，别去查业务逻辑

场景（2026-08-22）：用户报「切 Claude profile，第一次模型跟着变，后面的切换都不变」。
我**连续三次靠读代码猜错**：

1. 猜「模型下拉不认 profile 的槽位重映射」→ 用户截图证明列表里明明有 `deepseek-v4-pro[1m]`
   和「Custom Sonnet model」，映射显示得好好的
2. 猜「`selectorsCache` 按 agentType 键导致列表陈旧」→ 那确实是个结构缺陷，但不是本症状
3. 猜「`handledStaleRef` 认领了跃迁不复位」→ 重连会把 `configStale` 置 false，ref 跟着复位，路是通的

**实测十分钟就定位了。** CDP 连真实 WebView2，切三次，比对后端日志：只有第一次有
`disconnect` + `spawning`。数据库每次都正确更新，DOM 里查不到 stale 横幅
——**说明后端返回了 `affected=0`，根本没找到活连接**。

根因：`SessionState::conversation_id` 在生产代码里**只由 `ConversationLinked` /
`ConversationForked` 赋值**（`session_state.rs:1006/1016`），这两个事件在**会话行被创建**
时才发。重连到已存在的行不发 → 重启出来的新连接是**未绑定**的 → 之后所有
「按 conversation 扫描连接」的查找全部落空。

规则一：**「第一次生效、之后不生效」这个形状，先怀疑"重建之后某个索引/绑定没重新建立"，
不要先去查业务分支。** 业务逻辑坏了通常是「一次都不生效」或「随机不生效」，
不会这么整齐地只放过第一次。

规则二：**注意区分"方向"和"次序"。** 我一开始把它读成「切回去时不更新」（方向），
用户纠正说反过来也一样。重测发现第 3 次和第 1 次方向相同却失败——是**次序**。
方向假设会把你引向"某个分支没处理"，次序假设直接指向"状态被消耗了"。

规则三：**静默成功比报错难查十倍。** 这条链上前端看到 `affectedRunningSessions < 1`
就弹绿色「切换成功」。用户看到的是成功，实际什么都没做。
写这类"找不到目标就当没事发生"的分支时，想清楚它是"确实没事"还是"我没找到"。
