...
最后更新：2026-08-21 深夜（配置优先级倒挂定性 + 后台唤醒缺口立项）
...
- [x] **会话级开关「应用项目自带的 .claude 配置」** —— commit `7a86e751`。
      键 `__codeg_project_settings__`（`"off"` 才是关，缺省=开，零迁移）。
      开关放在启动档下拉里，不另起 chip。门禁绿：Rust **2775 passed**、
      前端 **391 files / 4975 tests**
