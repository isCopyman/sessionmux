...
> **2026-08-22 后续核验（不要再把下面的历史快照当当前缺口）**：产品裁决已经落地：
> `Ctrl+K` 现为纯标题快速跳转并引导到会话中心；会话中心已经混排 Room 标题并支持
> 标题/正文范围切换。此前不是索引损坏，而是 `C:\Users\63036\.local\bin\ctx.exe`
> 丢失；按既有 3 GB 索引的版本状态恢复官方 `ctx 0.25.0` 后，Codeg 的原调用协议
> 可直接工作。真实 Tauri WebView2 搜索 `profile 切换` 返回 50 条正文命中，未出现
> `contentSearchUnavailable`。下文保留为 08-20 当时的代码与机器状态审计。
