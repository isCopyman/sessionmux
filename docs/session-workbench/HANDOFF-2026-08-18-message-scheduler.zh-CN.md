...
4. **timer 的 auto_pause 永不动 enabled**；用户 update 无条件清 strike/auto_pause——前端
   "恢复"按钮靠这个，别在后端加条件。（已随 34ae1c36 机制拆除而作废，2026-08-19 注：
   auto_pause 仅剩测试引用的遗留代码。）
5. **strike 只在"无新信息且有未回义务"时累积**；纯本地干活的 goal 循环保持原行为
   （宿主判不准"本地零进展"，误刹更伤；转写增量启发式记为 v2）。（已随 34ae1c36 机制
   拆除而作废，2026-08-19 注：strike 现随未 reset 的 fire 累积，与出站义务无关。）
