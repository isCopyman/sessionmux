...
import { useTranscriptStickToBottom } from "./transcript-stick-to-bottom"
...
export type TranscriptScrollApi = MessageScrollContextValue
...
function VirtualizedTranscriptImpl<T>({
  items,
  getItemKey,
  renderItem,
  emptyState,
  ...
  const { scrollRef, stopScroll } = useTranscriptStickToBottom()
  ...
  const scrollToIndex = useCallback<TranscriptScrollApi["scrollToIndex"]>(
    (index, opts) => {
...
export const VirtualizedTranscript = memo(
  VirtualizedTranscriptImpl
) as typeof VirtualizedTranscriptImpl
