...
#[cfg(test)]
use super::acp_connect::resolve_connect_binding;
pub(crate) use super::acp_connect::{acp_connect_core, AcpConnectRequest};
...
fn is_volatile_fingerprint_key(key: &str) -> bool {
    // OPENCLAW_RESET_SESSION: per-launch, would flip the fingerprint the
    // moment a session id is assigned.
    // CLAUDE_CONFIG_DIR: per-conversation profile pin. Agent-level fingerprint
...
pub async fn acp_cancel(
    ...
    acp_cancel_core(&db, &manager, &EventEmitter::Tauri(app), &connection_id).await
}
...
"#;
    ...
    fn fingerprint_config_is_deterministic_and_excludes_volatile_keys() {
        ...
        temp_env::with_var("CODEX_HOME", Some(dir.path()), || {
            ...
            let mut env_profile = env.clone();
            env_profile.insert(
                "CLAUDE_CONFIG_DIR".to_string(),
                "/tmp/claude-profiles/x".to_string(),
            );
            assert_eq!(
                fp1,
