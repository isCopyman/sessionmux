...
impl CodexParser {
    ...
    fn parse_jsonl_summary(
        ...
        let mut harness_internal = false;
                ...
                "session_meta" => {
                    if let Some(payload) = value.get("payload") {
                        ...
                        git_branch = payload
                            ...
                            .and_then(|b| b.as_str())
                            .map(|s| s.to_string());
                        harness_internal = codex_session_is_harness_internal(payload);
                    }
                }
...
/// Codex's own one-turn subagents write a separate `rollout-*.jsonl` whose
/// `session_meta` is marked `thread_source=subagent` (and usually also carries
/// `parent_thread_id` / `source.subagent.thread_spawn`). Those are Harness
/// execution details, not user or Codeg-owned Sessions.
pub(crate) fn codex_session_is_harness_internal(payload: &serde_json::Value) -> bool {
    if payload
        .get("thread_source")
        .and_then(|value| value.as_str())
...
mod tests {
    ...
    #[test]
    fn detects_codex_harness_internal_session_meta() {
        assert!(super::codex_session_is_harness_internal(
            &serde_json::json!({
                "id": "child",
    ...
    #[test]
    fn list_conversations_marks_internal_subagent_rollouts() {
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system time ok")
            .as_nanos();
        let dir = env::temp_dir().join(format!("codeg-codex-internal-{nanos}"));
        ...
        fs::write(
            &user,
            concat!(
                r#"{"timestamp":"2026-06-09T13:52:12.542Z","type":"session_meta","payload":{"id":"user-1","cwd":"/tmp/demo","thread_source":"user","source":"vscode"}}"#,
                "\n",
                r#"{"timestamp":"2026-06-09T13:52:13.000Z","type":"event_msg","payload":{"type":"user_message","message":"hello"}}"#,
                "\n",
            ),
        )
        .expect("write user");
        fs::write(
            &child,
            concat!(
                r#"{"timestamp":"2026-06-09T17:18:25.007Z","type":"session_meta","payload":{"id":"child-1","cwd":"/tmp/demo","thread_source":"subagent","parent_thread_id":"user-1","source":{"subagent":{"thread_spawn":{"parent_thread_id":"user-1"}}}}}"#,
                "\n",
                r#"{"timestamp":"2026-06-09T17:18:26.000Z","type":"event_msg","payload":{"type":"user_message","message":"explore"}}"#,
                "\n",
            ),
        )
        .expect("write child");
        ...
        let user_row = listed.iter().find(|row| row.id == "user-1").expect("user");
        let child_row = listed
            .iter()
            .find(|row| row.id == "child-1")
            .expect("child");
        assert!(!user_row.harness_internal);
        assert!(child_row.harness_internal);
    ...
    fn session_index_title_overrides_rollout_title_in_list_and_detail() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let codex_home = env::temp_dir().join(format!("codeg-codex-session-index-{nanos}"));
